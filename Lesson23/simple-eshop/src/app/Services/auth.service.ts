...
export class AuthService {
  ...
  //Funkcia pre testovanie JWTInterceptor zadanie 23
  getTestData() {
  ...
  addCard(cardForm: FormGroup, rememberMe: boolean){
    const currentUser = this.user.value
    const isRememberMeCard = rememberMe
    let newCard = {}
    if(isRememberMeCard){
      newCard = {
        id: Date.now(),
        name: cardForm.value.fullName.split(' ')[0],
        surname: cardForm.value.fullName.split(' ')[1],
        cardNumber: cardForm.value.cardNumber.replace(/\s+/g, ''),
        expirationDate: `${cardForm.value.month}/${cardForm.value.year}`,
  ...
  onSaveOrderToUser(orderData: {}){
    ...
    this.firestore.collection('users').doc(currentUser?.id).update({
      ...
      this.snackBarService.showSnackBar('Failed to save order', 'error')
    });
  }
